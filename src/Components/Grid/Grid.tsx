...
import {v4} from 'uuid';
import {MLGridItem, MobileGridItem, WebDevGridItem} from '../GridItem/GridItems';
import './Grid.css';
...
export type MobileProject = Project & {
    type: 'mobile',
    link: string,
    image: string
}
...
export function MobileGrid({data}: MobileGridProps) {
    // const [showMore, setShowMore] = useState<boolean>(false);
    ...
    return (
        ...
        <div className="grid-wrapper">
            ...
            <div className="grid mobile-grid">
                {/* {data.map(details => { */}
                {data.slice(0,14).map(details => {
                    return <MobileGridItem key={v4()} details={details} />
                })}
